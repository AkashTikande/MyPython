...
#date 12/03/2020
# date 15/03/2020
#    are check in and check out dates accepted only.
#    there is no data storing system,
#    multiple pages are not linked with each other.
#    have to link resistration
#    booking details
#    payments
#    services
#    with each other
#    by using looping or onclick()#
import random
import datetime
...
def Booking():
    ...
    date(co)
    d1 = datetime.datetime(ci[2],ci[1],ci[0])  # date - 12/03/2020  by default
    d2 = datetime.datetime(co[2],co[1],co[0])  # date - 12/03/2020  by default24
    d = (d2-d1).days
    day.append(d)
